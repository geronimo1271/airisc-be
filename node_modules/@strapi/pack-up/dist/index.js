...
const loadConfig = async ({ cwd, logger }) => {
  ...
  for (const fileName of CONFIG_FILE_NAMES) {
    ...
    if (exists) {
      ...
      unregister();
      const config = (mod == null ? void 0 : mod.default) || mod || void 0;
      if (config) {
...
const createBuildContext = async ({
  ...
  const targets = {
    ...
    node: browserslistToEsbuild__default.default(["node 16.0.0"]),
    web: ["esnext"]
  };
  ...
  const outputPaths = Object.values(exports2).flatMap((exportEntry) => {
    ...
    return [
      exportEntry.import,
      exportEntry.require,
      (_a = exportEntry.browser) == null ? void 0 : _a.import,
      (_b = exportEntry.browser) == null ? void 0 : _b.require,
      ((_c = exportEntry.node) == null ? void 0 : _c.source) && exportEntry.node.import,
      ((_d = exportEntry.node) == null ? void 0 : _d.source) && exportEntry.node.require
    ].filter(Boolean);
  ...
  const configDistPath = (config == null ? void 0 : config.dist) ? path__namespace.default.resolve(cwd, config.dist) : void 0;
  ...
  return {
    ...
    logger,
    pkg,
    runtime: config == null ? void 0 : config.runtime,
    targets,
    ts: tsConfig
  };
};
...
const createTasks = (mode) => async (ctx) => {
  ...
  for (const exp of exps) {
    ...
    if ((_a = exp.browser) == null ? void 0 : _a.require) {
      createViteTask("cjs", "web", {
        path: exp._path,
        entry: ((_b = exp.browser) == null ? void 0 : _b.source) || exp.source,
        output: exp.browser.require
      });
    }
    if ((_c = exp.browser) == null ? void 0 : _c.import) {
      createViteTask("cjs", "web", {
        path: exp._path,
        entry: ((_d = exp.browser) == null ? void 0 : _d.source) || exp.source,
...
const resolveViteConfig = (ctx, task) => {
  ...
  const config = {
    ...
    build: {
      ...
      rollupOptions: {
        external(id, importer) {
          if (exportIds == null ? void 0 : exportIds.includes(id)) {
            return true;
          }
          if (importer && (id.startsWith(".") || id.startsWith("/"))) {
            const idPath = path__namespace.default.resolve(path__namespace.default.dirname(importer), id);
            if (sourcePaths == null ? void 0 : sourcePaths.includes(idPath)) {
              ctx.logger.warn(
     ...
     */
    plugins: runtime === "node" ? [] : [react__default.default()]
  };
  return config;
};
const viteBuildTask = {
  ...
  if (Array.isArray(template.prompts)) {
    for (const prompt of template.prompts) {
      if ("type" in prompt) {
        ...
        answers.push({
          name: prompt.name,
          answer: (res == null ? void 0 : res.confirm) ?? !prompt.optional
        });
      }
    }
    logger.debug(
  ...
  return {
    prompts: [
      ...
      definePackageOption({
        ...
        initial: () => (repo == null ? void 0 : repo.name) ?? "",
        validate(v) {
          if (!v) {
      ...
      definePackageOption({
        ...
        initial: (_a = gitConfig == null ? void 0 : gitConfig.user) == null ? void 0 : _a.name
      }),
      definePackageOption({
        name: "authorEmail",
        type: "text",
        message: "package author email",
        initial: (_b = gitConfig == null ? void 0 : gitConfig.user) == null ? void 0 : _b.email
      }),
      definePackageOption({
        name: "license",
        type: "text",
